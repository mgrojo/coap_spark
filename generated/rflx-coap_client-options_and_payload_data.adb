...
package body RFLX.CoAP_Client.Options_And_Payload_Data with
  SPARK_Mode
is
   ...
   function Get (Ctx : Context; Fld : Field) return RFLX_Types.Base_Integer with
     Pre =>
       ...
       and then RFLX.CoAP_Client.Options_And_Payload_Data.Sufficient_Buffer_Length (Ctx, Fld)
       and then not RFLX.CoAP_Client.Options_And_Payload_Data.Composite_Field (Fld)
   is
   ...
   procedure Set (Ctx : in out Context; Fld : Field; Val : RFLX_Types.Base_Integer; Size : RFLX_Types.Bit_Length; State_Valid : Boolean; Buffer_First : out RFLX_Types.Index; Buffer_Last : out RFLX_Types.Index; Offset : out RFLX_Types.Offset) with
     Pre =>
       RFLX.CoAP_Client.Options_And_Payload_Data.Has_Buffer (Ctx)
       and then RFLX.CoAP_Client.Options_And_Payload_Data.Valid_Next (Ctx, Fld)
       and then RFLX.CoAP_Client.Options_And_Payload_Data.Valid_Value (Fld, Val)
       and then RFLX.CoAP_Client.Options_And_Payload_Data.Valid_Size (Ctx, Fld, Size)
       and then Size <= RFLX.CoAP_Client.Options_And_Payload_Data.Available_Space (Ctx, Fld)
       and then (if
                    RFLX.CoAP_Client.Options_And_Payload_Data.Composite_Field (Fld)
                 then
   ...
   procedure Set_Scalar (Ctx : in out Context; Fld : Field; Val : RFLX_Types.Base_Integer) with
     Pre =>
       ...
       and then RFLX.CoAP_Client.Options_And_Payload_Data.Valid_Value (Fld, Val)
       and then RFLX.CoAP_Client.Options_And_Payload_Data.Valid_Size (Ctx, Fld, RFLX.CoAP_Client.Options_And_Payload_Data.Field_Size (Ctx, Fld))
       and then RFLX.CoAP_Client.Options_And_Payload_Data.Available_Space (Ctx, Fld) >= RFLX.CoAP_Client.Options_And_Payload_Data.Field_Size (Ctx, Fld)
       and then RFLX.CoAP_Client.Options_And_Payload_Data.Field_Size (Ctx, Fld) in 1 .. RFLX_Types.Base_Integer'Size
       and then RFLX_Types.Fits_Into (Val, Natural (RFLX.CoAP_Client.Options_And_Payload_Data.Field_Size (Ctx, Fld))),
     Post =>
       Has_Buffer (Ctx)
       and Valid (Ctx, Fld)
   ...
   begin
      ...
      pragma Assert (RFLX.CoAP_Client.Options_And_Payload_Data.Valid_Length (Ctx, RFLX.CoAP_Client.Options_And_Payload_Data.F_Options_And_Payload, Length));
