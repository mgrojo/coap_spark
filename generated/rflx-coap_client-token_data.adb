...
package body RFLX.CoAP_Client.Token_Data with
  SPARK_Mode
is
   ...
   function Get (Ctx : Context; Fld : Field) return RFLX_Types.Base_Integer with
     Pre =>
       ...
       and then RFLX.CoAP_Client.Token_Data.Sufficient_Buffer_Length (Ctx, Fld)
       and then not RFLX.CoAP_Client.Token_Data.Composite_Field (Fld)
   is
   ...
   begin
      ...
      then
         if Sufficient_Buffer_Length (Ctx, Fld) then
            ...
            then
               pragma Assert ((if Fld = F_Token then Field_Last (Ctx, Fld) mod RFLX_Types.Byte'Size = 0));
   ...
   procedure Set (Ctx : in out Context; Fld : Field; Val : RFLX_Types.Base_Integer; Size : RFLX_Types.Bit_Length; State_Valid : Boolean; Buffer_First : out RFLX_Types.Index; Buffer_Last : out RFLX_Types.Index; Offset : out RFLX_Types.Offset) with
     Pre =>
       RFLX.CoAP_Client.Token_Data.Has_Buffer (Ctx)
       and then RFLX.CoAP_Client.Token_Data.Valid_Next (Ctx, Fld)
       and then RFLX.CoAP_Client.Token_Data.Valid_Value (Fld, Val)
       and then RFLX.CoAP_Client.Token_Data.Valid_Size (Ctx, Fld, Size)
       and then Size <= RFLX.CoAP_Client.Token_Data.Available_Space (Ctx, Fld)
       and then (if
                    RFLX.CoAP_Client.Token_Data.Composite_Field (Fld)
                 then
     ...
     Post =>
       ...
       and then (Ctx.Cursors (Fld).Value = Val
                 and then (if Fld in F_Token and then Well_Formed_Message (Ctx) then Message_Last (Ctx) = Field_Last (Ctx, Fld)))
   ...
   procedure Set_Scalar (Ctx : in out Context; Fld : Field; Val : RFLX_Types.Base_Integer) with
     Pre =>
       ...
       and then RFLX.CoAP_Client.Token_Data.Valid_Next (Ctx, Fld)
       and then Fld in F_Length | F_Unused
       and then RFLX.CoAP_Client.Token_Data.Valid_Value (Fld, Val)
       and then RFLX.CoAP_Client.Token_Data.Valid_Size (Ctx, Fld, RFLX.CoAP_Client.Token_Data.Field_Size (Ctx, Fld))
       and then RFLX.CoAP_Client.Token_Data.Available_Space (Ctx, Fld) >= RFLX.CoAP_Client.Token_Data.Field_Size (Ctx, Fld)
       and then RFLX.CoAP_Client.Token_Data.Field_Size (Ctx, Fld) in 1 .. RFLX_Types.Base_Integer'Size
       and then RFLX_Types.Fits_Into (Val, Natural (RFLX.CoAP_Client.Token_Data.Field_Size (Ctx, Fld))),
     Post =>
       Has_Buffer (Ctx)
       and Valid (Ctx, Fld)
   ...
   procedure Set_Length (Ctx : in out Context; Val : RFLX.CoAP.Token_Length) is
   begin
      Set_Scalar (Ctx, F_Length, RFLX.CoAP.To_Base_Integer (Val));
   end Set_Length;

   procedure Set_Unused (Ctx : in out Context; Val : RFLX.CoAP_Client.Unused_4) is
   begin
      Set_Scalar (Ctx, F_Unused, RFLX.CoAP_Client.To_Base_Integer (Val));
   end Set_Unused;
   ...
   begin
      ...
      pragma Assert (RFLX.CoAP_Client.Token_Data.Valid_Length (Ctx, RFLX.CoAP_Client.Token_Data.F_Token, Length));
   ...
   begin
      ...
      Struct.Unused := Get_Unused (Ctx);
      Struct.Token := (others => 0);
      Get_Token (Ctx, Struct.Token (Struct.Token'First .. Struct.Token'First + RFLX_Types.Index (RFLX_Types.To_Length (Field_Size (Ctx, F_Token)) + 1) - 2));
   end To_Structure;
   ...
   begin
      ...
      Set_Unused (Ctx, Struct.Unused);
      Set_Token (Ctx, Struct.Token (Struct.Token'First .. Struct.Token'First + RFLX_Types.Index (RFLX_Types.To_Length (RFLX_Types.Bit_Length (Struct.Length) * 8) + 1) - 2));
   end To_Context;
